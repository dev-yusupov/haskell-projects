...
getLevel _ Leaf = -1
getLevel x (Node y l r)
    | x == y = 0
    | otherwise =
        let leftLevel = getLevel x l
            rightLevel = getLevel x r

        in if leftLevel /= -1 then 1 + leftLevel
        else if rightLevel /= -1 then 1 + rightLevel
        else -1
...
nodeCountLevel (Node {}) 1 = 1
nodeCountLevel (Node _ l r) n
