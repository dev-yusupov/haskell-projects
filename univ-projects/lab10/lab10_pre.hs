...
-- 1. Search for a value in the tree
...
task2 Leaf _ = 0
task2 tr n = length [x | x <- inorder tr, x == n]

--recursion method
task21 :: Tree Int -> Int -> Int
task21 Leaf _ = 0
task21 (Node x left right) n
    | x == n = 1 + task2 left n + task2 right n
    | otherwise = task2 left n + task2 right n

main = do
    print $ task2 tree3fiveTimes 3 -- 5
    print $ task2 tree2 (-10) -- 0

-- 3. Check if every element from the list is in the tree
...
-- 4. Create list of tuples (node, left child, right child) for odd numbers
...
-- 6. Replace nodes equal to n with 0
...
-- 7. Add "_over18" to names of persons over 18
data Person = Person {
    name :: String,
