...
task1 tree list = all (`elem` treeElements) list
    where treeElements = inorder tree
...
task2 tree y = length $ filter (== y) (inorder tree)

task21 :: Tree Int -> Int -> Int
task21 Leaf _ = 0
task21 (Node x left right) y
    | x == y = 1 + task21 left y + task21 right y
    | otherwise = task21 left y + task21 right y
...
task3 (Node x left right)
    | odd x = [(x, extractN left, extractN right)] ++ task3 left ++ task3 right
    | otherwise = task3 left ++ task3 right
...
searchT (Node x left right) n
    | x == n = True
    | otherwise = searchT left n || searchT right n
...
f8 :: Tree Int -> Int -> Int
f8 Leaf _ = 0
f8 (Node x left right) n 
    | x == n = exNode left + exNode right + f8 left n + f8 right n
    | otherwise = f8 left n + f8 right n
...
replace n Leaf = Leaf
replace n (Node x left right)
    | n == x = Node 0 (replace n left) (replace n right)
    | otherwise = Node x (replace n left) (replace n right)
...
updateName (Node x left right)
    | over18 (birthday x) = Node (addString x) (updateName left) (updateName right)
    | otherwise = Node x (updateName left) (updateName right)

-- main = print $ updateName t1
-- main = print $ updateName t2
-- main = print $ updateName t3

-- 13. Binary Search Tree check
